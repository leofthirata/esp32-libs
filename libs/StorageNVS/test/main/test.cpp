...
Storage::Storage *storage2 = nullptr;
...
extern "C" void app_main(void)
{
    ...
    storage->otp_set(&config);
    storage->otp_get(&temp);
    ...
    storage->status_set(&config);
    storage->status_get(&temp);
    ...
    storage->ble_lora_gsm_set(&config);
    storage->ble_lora_gsm_get(&temp);
    ...
    storage->config_set(&config);
    storage->config_get(&temp);
    ...
    uint32_t id = 0;
    storage->gsm_save_position(gsmJson, &id);

    char gsmJsonRead[150];
    size_t len = 200;
    esp_err_t err = storage->gsm_get_last_position(gsmJsonRead, &len);
    ...
    err = storage->gsm_get_last_position(gsmJsonRead2, &len);
    ESP_LOGI(TAG, "gsmJsonRead2 (%s)\n", gsmJsonRead2);
    printf("len: %d\r\n", len);

    // char gsmJsonRead3[200];
    // err = storage->gsm_get_last_position(gsmJsonRead3, &len);
    // ESP_LOGI(TAG, "gsmJsonRead3 (%s)\n", gsmJsonRead3);
    // printf("len: %d\r\n", len);

    // char gsmJsonRead4[200];
    // err = storage->gsm_get_last_position(gsmJsonRead4, &len);
    // ESP_LOGI(TAG, "gsmJsonRead4 (%s)\n", gsmJsonRead4);
    // printf("len: %d\r\n", len);

    // char gsmJsonRead5[200];
    // err = storage->gsm_get_last_position(gsmJsonRead5, &len);
    // if (err != ESP_OK)
    ...
    storage2 = new Storage::Storage();
    ESP_ERROR_CHECK(storage2->init());

    char gsmJsonRead3[200];
    err = storage2->gsm_get_last_position(gsmJsonRead3, &len);
    ESP_LOGI(TAG, "gsmJsonRead3 (%s)\n", gsmJsonRead3);
    printf("len: %d\r\n", len);
}
